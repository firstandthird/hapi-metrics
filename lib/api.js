  ...
  server.route({
    ...
    handler: function(request, reply) {
      ...
      collection.aggregate([
        { $match: query },
        { $group: {
            _id : { month: { $month: '$date' }, day: { $dayOfMonth: '$date' }, year: { $year: '$date' }, hour: { $hour: '$date' }, distinct: distinct },
            count: { $sum: 1 }
          }
        },
        ...
        var out = results.map(function(result) {
          var o = {
            date: new Date(result._id.year, result._id.month - 1, result._id.day, result._id.hour),
            value: result.count
          };
